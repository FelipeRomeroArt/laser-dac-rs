...
- `NetworkFifo` — FIFO-buffered DACs with a queryable queue depth (e.g. Ether Dream, LaserCube WiFi, AVB). The scheduler estimates outstanding points and tops up to a target buffer.
- `UdpTimed` — DACs paced by timed UDP chunks where OS send time, not hardware position, is the clock (e.g. IDN). The scheduler emits fixed-duration chunks against a precise deadline.
